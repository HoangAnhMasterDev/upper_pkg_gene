...
//         RCLCPP_INFO(get_logger(), "Upper command topic: %s", upper_command_topic_.c_str());
//         RCLCPP_INFO(get_logger(), "Publishing /joint_states for RViz.");
//         RCLCPP_INFO(get_logger(), "Publishing /upper_motor/state for upper momentum compensator.");
//     }
...
//         command_to_urdf_index_["L_shoulder_pitch"] = indexOf("L_shoulder_pitch_joint");
//         command_to_urdf_index_["L_shoulder_roll"]  = indexOf("L_shoulder_roll_joint");
//         command_to_urdf_index_["L_shoulder_yaw"]   = indexOf("L_shoulder_yaw_joint");
//         command_to_urdf_index_["L_elbow"]          = indexOf("L_elbow_joint");

//         command_to_urdf_index_["R_shoulder_pitch"] = indexOf("R_shoulder_pitch_joint");
//         command_to_urdf_index_["R_shoulder_roll"]  = indexOf("R_shoulder_roll_joint");
//         command_to_urdf_index_["R_shoulder_yaw"]   = indexOf("R_shoulder_yaw_joint");
//         command_to_urdf_index_["R_elbow"]          = indexOf("R_elbow_joint");

//         command_to_urdf_index_["L_hip"]    = indexOf("L_hip_joint");
//         command_to_urdf_index_["L_hip2"]   = indexOf("L_hip2_joint");
//         command_to_urdf_index_["L_thigh"]  = indexOf("L_thigh_joint");
//         command_to_urdf_index_["L_calf"]   = indexOf("L_calf_joint");
//         command_to_urdf_index_["L_toe"]    = indexOf("L_toe_joint");

//         command_to_urdf_index_["R_hip"]    = indexOf("R_hip_joint");
//         command_to_urdf_index_["R_hip2"]   = indexOf("R_hip2_joint");
//         command_to_urdf_index_["R_thigh"]  = indexOf("R_thigh_joint");
//         command_to_urdf_index_["R_calf"]   = indexOf("R_calf_joint");
//         command_to_urdf_index_["R_toe"]    = indexOf("R_toe_joint");
...
//             std::distance(urdf_joint_names_.begin(), it));
//     }
...
//         upper_state_pub_ =
//             create_publisher<UpperStateMsg>(
//                 "/upper_motor/state",
//                 rclcpp::QoS(10));
...
//                 rclcpp::QoS(10),
//                 std::bind(
//                     &WholeBodyPseudoDriverNode::upperCommandCallback,
//                     this,
//                     std::placeholders::_1));
...
//             create_wall_timer(
//                 period,
//                 std::bind(&WholeBodyPseudoDriverNode::publishFeedback, this));
//     }
...
//         applyNamedCommand(*msg, now_sec);

//         has_command_ = true;
//     }

//     void upperCommandCallback(const CommandMsg::SharedPtr msg)
//     {
//         const double now_sec = now().seconds();
...
//         {
//             applyUpperOrderCommand(*msg, now_sec);
//         }
...
//                 dq_[urdf_idx] =
//                     std::clamp(dq_raw, -max_fake_dq_, max_fake_dq_);
//             }
//         }
...
//     {
//         const size_t n =
//             std::min(msg.state.name.size(), msg.state.position.size());

//         for (size_t i = 0; i < n; ++i)
//         {
//             const auto it =
//                 command_to_urdf_index_.find(msg.state.name[i]);
...
//     void applyUpperOrderCommand(
//         const CommandMsg& msg,
//         double now_sec)
//     {
//         const size_t n =
//             std::min<size_t>(8, msg.state.position.size());

//         for (size_t i = 0; i < n; ++i)
//         {
//             const std::string& cmd_name = upper_command_names_[i];

//             const int urdf_idx =
//                 command_to_urdf_index_[cmd_name];
...
//     void publishUpperState()
//     {
//         UpperStateMsg msg;

//         msg.joint_names = upper_state_joint_names_;
//         msg.position.assign(upper_state_joint_names_.size(), 0.0);
//         msg.velocity.assign(upper_state_joint_names_.size(), 0.0);
//         msg.torque_cmd.assign(upper_state_joint_names_.size(), 0.0);

//         for (size_t i = 0; i < upper_state_joint_names_.size(); ++i)
//         {
//             const int idx = indexOf(upper_state_joint_names_[i]);

//             if (idx < 0 || idx >= static_cast<int>(q_.size()))
//             {
//                 continue;
//             }

//             msg.position[i] = q_[idx];
//             msg.velocity[i] = dq_[idx];
//             msg.torque_cmd[i] = tau_[idx];
//         }

//         msg.fault = false;
//         msg.fault_status = false;
//         msg.fault_limit = false;
//         msg.fault_command_timeout = false;

//         upper_state_pub_->publish(msg);
//     }
...
//     std::string upper_command_topic_;
...
//     std::vector<std::string> upper_command_names_;
...
//     rclcpp::Publisher<UpperStateMsg>::SharedPtr upper_state_pub_;

//     rclcpp::Subscription<CommandMsg>::SharedPtr lower_command_sub_;
//     rclcpp::Subscription<CommandMsg>::SharedPtr upper_command_sub_;
...
//     rclcpp::spin(std::make_shared<WholeBodyPseudoDriverNode>());
//     rclcpp::shutdown();
//     return 0;
// }

// Below is the offset version, and up there is non-offset version. The offset version is used to make the humanoid pseudo driver more realistic, and the non-offset version is used to test the upper momentum compensator.
// because we have dq_cmd from momentum, so dq_feedback = dq cmd
#include <rclcpp/rclcpp.hpp>
#include <sensor_msgs/msg/joint_state.hpp>
...
private:
    ...
    void initBuffers()
    {
         ...
         *
         * Current sign convention from your latest code:
         *
    ...
    void lowerCommandCallback(const CommandMsg::SharedPtr msg)
    {
        ...
        if (!msg->state.name.empty())
        {
             ...
             * Apply hardware-to-URDF offset for lower-body visualization.
             */
            applyNamedCommand(
                *msg,
                now_sec,
    ...
    void upperCommandCallback(const CommandMsg::SharedPtr msg)
    {
        ...
        {
            /*
             * Upper command with names.
             * Do NOT apply lower-body offset.
             */
            applyNamedCommand(
                *msg,
                now_sec,
        ...
        {
            /*
             * Upper command without names.
             * Assume upper hardware order:
             *
             * 0 L_shoulder_pitch
             * 1 L_shoulder_roll
             * 2 L_shoulder_yaw
        ...
        bool apply_lower_offset)
    {
        ...
        for (size_t i = 0; i < n; ++i)
        {
             ...
             *
             * For lower body:
             *   q_feedback_visual = q_cmd + hardware_to_URDF_offset
             *
             * For upper body:
             *   q_feedback_visual = q_cmd
             */
            double q_visual = msg.state.position[i];

            if (apply_lower_offset)
            {
                q_visual += q_cmd_offset_[urdf_idx];
            }
            ...
            /*
             * dq_feedback = dq_cmd if velocity is provided.
             *
             * Offset is constant, so:
             *   d(q_cmd + offset)/dt = dq_cmd
             */
            if (has_velocity)
            {
        ...
        double now_sec)
    {
         ...
         * 9 R_toe
         */
        static const std::vector<std::string> lower_command_names = {
            "R_hip",
            "R_hip2",
            "R_thigh",
            "R_calf",
        ...
        for (size_t i = 0; i < n; ++i)
        {
            ...
            const double q_visual =
                msg.state.position[i] + q_cmd_offset_[urdf_idx];
        ...
        double now_sec)
    {
        ...
        for (size_t i = 0; i < n; ++i)
        {
            const std::string& cmd_name =
                upper_command_names_[i];

            const auto it =
                command_to_urdf_index_.find(cmd_name);
             ...
             * Upper body:
             *   q_feedback = q_cmd
             *   dq_feedback = dq_cmd if available
             */
            const bool has_velocity =
                i < msg.state.velocity.size();
    ...
    void publishUpperState()
    {
        ...
        {
            const int idx =
                indexOf(upper_state_joint_names_[i]);
