...
from upper_motor_bridge.msg import UpperMotorState
import rclpy
...
class DataRecorder(Node):
    def __init__(self):
        ...
        # Output path
        # =============================
        self.results_dir = os.path.expanduser("~/upper_ws/robot_test_data")
        ...
        self.latest_torque_cmd = [0.0] * self.num_joints
        ...
        self.sub_joint_state = self.create_subscription(
            JointState, '/upper_motor/joint_states', self.joint_state_callback, 10)
        ...
        self.sub_joint_cmd = self.create_subscription(
            CustomJointState, '/upper_motor/command', self.joint_cmd_callback, 10)

        self.sub_torque = self.create_subscription(
            UpperMotorState, '/upper_motor/state', self.torque_callback, 10)

        # self.sub_torque_fb = self.create_subscription(
        #     JointState, '/upper_motor/joint_states_fb_torque', self.torque_fb_callback, 10)
    ...
    def torque_callback(self, msg: UpperMotorState):
        self.latest_torque_fb = self._resize_or_pad(msg.feedback_torque, self.num_joints)
        self.latest_torque_cmd = self._resize_or_pad(msg.torque_cmd, self.num_joints)
    ...
    def record_snapshot(self):
        ...
        self.data['torque_fb'].append(self.latest_torque_fb.copy())
        self.data['torque_cmd'].append(self.latest_torque_cmd.copy())
    ...
    def save_to_csv(self):
        ...
        try:
            with open(self.csv_file, 'w', newline='') as f:
                ...
                header.extend([f'{name}_torque_fb' for name in self.joint_names])
                header.extend([f'{name}_torque_cmd' for name in self.joint_names])
                ...
                for i in range(num_samples):
                    ...
                    row.extend(self.data['torque_fb'][i])
                    row.extend(self.data['torque_cmd'][i])
    ...
    def plot_data(self):
        ...
        try:
            ...
            torque_fb = np.array(self.data['torque_fb'])
            torque_cmd = np.array(self.data['torque_cmd'])
            ...
            for row in range(2):
                for col in range(4):
                    ...
                    ax.plot(times, torque_cmd[:, data_idx], label='torque command')
                    ax.plot(times, torque_fb[:, data_idx], label='torque feedback')
                    ax.set_title(joint_name)
                    ax.set_xlabel('Time (s)')
                    ax.set_ylabel('Torque (Nm)')
                    ax.grid(True)
